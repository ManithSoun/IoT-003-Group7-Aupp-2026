...
The system uses DeepFace to detect 6 core emotions: Happy, Sad, Fear, Angry, and Surprised, plus Neutral as a calm default state. The buzzer plays a short notification tone when the emotion changes, while Spotify handles the background music through the laptop speakers.

In addition to automatic detection, the system also supports manual control via a Telegram bot and Web Dashboard, allowing the user to force a capture, check the current room state, or override the music at any time.

![Mood Room](/mood-room/assets/mood-room.JPG)
...
![Node A](/mood-room/assets/node-a-wiring.png)
...
![Node B](/mood-room/assets/node-b-wiring.png)

### Sensors:

| Component  | Pin    | Purpose                                                |
| ---------- | ------ | ------------------------------------------------------ |
| PIR sensor | GPIO13 | Door detection, triggers face scan                     |
| IR sensor  | GPIO12 | Room presence, monitors if person is still in the room |
...
8. IR sensor continues monitoring presense, if no motion for 20 seconds, everything turns off
...
```
mood-room/
├── esp32_actuator/            # RUN ON ESP32
│   ├── main.py                  # Main loop: WiFi, MQTT, PIR, emotion dispatch
│   ├── led.py                   # NeoPixel LED control
│   ├── buzzer.py                # PWM buzzer tone patterns
│   ├── display.py               # OLED display control
│   ├── motor.py                 # DC motor control via L298N
│   ├── pir.py                   # PIR and IR sensor reading
│   └── umqtt_simple.py          # Lightweight MQTT client for MicroPython
│
├── laptop/                    # RUN ON LAPTOP
│   ├── server.py                # Main server: Flask + DeepFace + MQTT + camera
│   ├── music.py                 # Spotify playlist switching
│   ├── telegram_bot.py          # Telegram bot commands and MQTT integration
│   ├── index.html               # Web dashboard for manual control
│
└── assets/
...
- Shows a startup screen on the OLED
- Connects to WiFi and displays the IP address on screen
- Connects to the HiveMQ MQTT broker
- Subscribes to `moodroom/emotion` to receive commands from the laptop
- Starts a background thread that polls for new MQTT messages every 50ms
- Main thread continuously reads both the PIR and IR sensors

**PIR Behavior:**

- Door PIR trigger:
  - Publish `moodroom/pir: detected`
  - Tells the laptop to start a face scan.
- Room IR monitoring (presence inside the room):
  - If no motion for >20 seconds → publish `moodroom/pir: left`
  - Shut down all actuators

## 4.3 Laptop-Side Software

### 4.3.1 `server.py` — Main Server

This is the main file that runs on the laptop. It handles everything together: receiving images from the ESP32-CAM, running DeepFace emotion detection, serving the web dashboard, and publishing results via MQTT.

**Emotion Detection — Two-Stage Pipeline:**

**Stage 1 — Face Detection:**
OpenCV uses a Haar Cascade classifier to find faces in the image. It selects the largest face found and adds 20px of padding around it to include the full face area before passing it to DeepFace.

**Stage 2 — Emotion Classification:**
DeepFace analyzes the cropped face and returns confidence scores for all 6 emotions. The system removes neutral from consideration first and picks the strongest remaining emotion. Neutral is only chosen if its score is more than 30% higher than the next emotion, this prevents the system from defaulting to neutral too easily.

**Smart Change Detection:**
If the detected emotion is the same as the current emotion, the system skips the update entirely. If no face is found, the current emotion is kept unchanged. This prevents the room from flickering unnecessarily.

### 4.3.2 `music.py` — Spotify Integration
...
### 4.3.3 `telegram_bot.py` — Remote Control Interface
...
| `/play_[emotion]` | Manual | Switch to manual mode and set emotion               |
| `/auto`           | Auto   | Restore automatic AI-based mode                     |
| `/room_off`       | Any    | Turn off all actuators and stop music               |
| `/help`           | Any    | Display all commands                                |

### 4.3.4 `index.html` — Web Dashboard

The web dashboard is a single HTML file served by Flask. It provides a live view of the room state and allows manual control without needing Telegram. It shows the current emotion, PIR and IR sensor status, song info, server connection status, and buttons for manual override.

## 5. Emotion Response Table
...
| Person left room                  | IR Room = LOW for >20 seconds               | Shut off all actuators, publish `moodroom/pir: left`   |
| Manual override                   | Telegram `/play` command or web dashboard   | Switch to manual mode; bypass PIR/AI pipeline entirely |
...
- The ESP32 flashed with MicroPython

### 7.2 Install Python Dependencies

```bash
pip install flask flask-cors deepface opencv-python spotipy paho-mqtt python-telegram-bot python-dotenv
```
...
After uploading, press the reset button on the ESP32. The OLED will show the IP address when it has connected to WiFi successfully.

---

### 7.5 Flash the ESP32-CAM

Open the ESP32-CAM sketch in Arduino IDE and set the upload speed to 115200 before flashing:

- Tools → Upload Speed → **115200**
- Tools → Board → **AI Thinker ESP32-CAM**
- Tools → Port → your port (example: `/dev/cu.usbserial-0001`)
...
**Solution:**  
Debouncing logic was added to filter out rapid repeated signals from the PIR sensor. Additionally, the system was updated to skip any update when the newly detected emotion matches the current emotion, so even if the sensor triggers multiple times, the room state only changes when something actually different is detected. This improved stability and reduced unnecessary processing.
